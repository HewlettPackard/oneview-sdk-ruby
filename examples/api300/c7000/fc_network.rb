...
fc = OneviewSDK::API300::C7000::FCNetwork.new(@client, options)
fc.create!
puts "\nCreated fc-network '#{fc[:name]}' sucessfully.\n  uri = '#{fc[:uri]}'"

# Find recently created network by name
matches = OneviewSDK::API300::C7000::FCNetwork.find_by(@client, name: fc[:name])
fc2 = matches.first
puts "\nFound fc-network by name: '#{fc2[:name]}'.\n  uri = '#{fc2[:uri]}'"

# Retrieve recently created network
fc3 = OneviewSDK::API300::C7000::FCNetwork.new(@client, name: fc[:name])
fc3.retrieve!
...
OneviewSDK::API300::C7000::FCNetwork.find_by(@client, attributes).each do |network|
  puts "  #{network[:name]}"
end
