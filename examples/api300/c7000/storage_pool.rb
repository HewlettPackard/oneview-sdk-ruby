...
# Create storage pool
item = OneviewSDK::API300::C7000::StoragePool.new(@client, options)
item.set_storage_system(storage_system)
item.add
puts "\nAdded #{type} '#{item[:name]}' sucessfully.\n  uri = '#{item[:uri]}'"

# verifying if storage pool exists
item_2 = OneviewSDK::API300::C7000::StoragePool.new(@client, name: options[:poolName], storageSystemUri: storage_system['uri'])
...
matches = OneviewSDK::API300::C7000::StoragePool.find_by(@client, name: item[:name])
