...
# Sucess case - 1
fcoe = OneviewSDK::API300::C7000::FCoENetwork.new(@client, options)
fcoe.create!
puts "\nCreated fcoe-network '#{fcoe[:name]}' sucessfully.\n  uri = '#{fcoe[:uri]}'"

# Find recently created network by name
matches = OneviewSDK::API300::C7000::FCoENetwork.find_by(@client, name: fcoe[:name])
fcoe2 = matches.first
puts "\nFound fcoe-network by name: '#{fcoe2[:name]}'.\n  uri = '#{fcoe2[:uri]}'"

# Retrieve recently created network
fcoe3 = OneviewSDK::API300::C7000::FCoENetwork.new(@client, name: fcoe[:name])
fcoe3.retrieve!
...
OneviewSDK::API300::C7000::FCoENetwork.find_by(@client, attributes).each do |network|
  puts "  #{network[:name]}"
end
