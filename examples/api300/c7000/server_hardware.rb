...
# Example: Add server hardware for an API300 C7000 Appliance
...
item = OneviewSDK::API300::C7000::ServerHardware.new(@client, options)
item.add
puts "\nAdded #{type} '#{item[:name]}' sucessfully.\n  uri = '#{item[:uri]}'"

# Find recently created item by name
matches = OneviewSDK::API300::C7000::ServerHardware.find_by(@client, name: item[:name])
item2 = matches.first
...
item3 = OneviewSDK::API300::C7000::ServerHardware.new(@client, name: item[:name])
item3.retrieve!
puts "\nFound #{type} by name: '#{item[:name]}'.\n  uri = '#{item3[:uri]}'"

# List all server hardware
puts "\n\n#{type.capitalize} list:"
OneviewSDK::API300::C7000::ServerHardware.find_by(@client, {}).each do |p|
  puts "  #{p[:name]}"
end

# Get a firmware inventory by id
item4 = OneviewSDK::API300::C7000::ServerHardware.find_by(@client, {}).first
