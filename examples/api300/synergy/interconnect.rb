...
OneviewSDK::API300::Synergy::Interconnect.find_by(@client, {}).each do |interconnect|
...
OneviewSDK::API300::Synergy::Interconnect.get_types(@client).each do |type|
  puts "Interconnect type #{type['name']} URI=#{type['uri']}"
end

item = OneviewSDK::API300::Synergy::Interconnect.find_by(@client, state: 'Configured').first
...
OneviewSDK::API300::Synergy::Interconnect.get_link_topologies(@client).each do |link_topology|
