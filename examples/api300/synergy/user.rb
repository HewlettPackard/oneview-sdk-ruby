...
# Creating a user
user = OneviewSDK::API300::Synergy::User.new(@client, options)
user.create
puts "\nCreated user '#{user[:userName]}' sucessfully.\n  uri = '#{user[:uri]}'"

# Find recently created network by userName
matches = OneviewSDK::API300::Synergy::User.find_by(@client, userName: user[:userName])
user2 = matches.first
...
OneviewSDK::API300::Synergy::User.get_all(@client).each do |u|
