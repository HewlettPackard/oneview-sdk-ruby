...
RSpec.describe klass, integration: true, type: CREATE, sequence: seq(klass) do
  ...
  describe '#exists?' do
    it 'returns true if storage pool exists' do
      storage_system = OneviewSDK::StorageSystem.new($client, storage_system_data)
      storage_system.retrieve!
      item = OneviewSDK::StoragePool.new($client, name: STORAGE_POOL_NAME, storageSystemUri: storage_system['uri'])
    ...
    it 'returns false if storage pool not exists' do
      storage_system = OneviewSDK::StorageSystem.new($client, storage_system_data)
      storage_system.retrieve!
      item = OneviewSDK::StoragePool.new($client, name: 'some unknown nama', storageSystemUri: storage_system['uri'])
    ...
    it 'should throw incomplete resource exception if name and uri or storageSystemUri are unknown' do
      storage_system = OneviewSDK::StorageSystem.new($client, storage_system_data)
      storage_system.retrieve!
      item_without_name_and_uri = OneviewSDK::StoragePool.new($client, storageSystemUri: storage_system['uri'])
      item_without_storage_system_uri = OneviewSDK::StoragePool.new($client, name: 'some unknown nama')
