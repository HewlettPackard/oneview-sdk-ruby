...
RSpec.describe OneviewSDK::API600::Synergy::LogicalInterconnect, integration: true, type: UPDATE do
  ...
  let(:interconnect_class) { OneviewSDK::API600::Synergy::Interconnect }
  let(:log_int_name) { LOG_INT2_NAME }
  let(:encl_name) { ENCL2_NAME }

  it_behaves_like 'LogicalInterconnectUpdateExample', 'integration api600 context'

  it_behaves_like 'ScopeHelperMethodsExample', OneviewSDK::API600::Synergy::Scope do
  ...
  it_behaves_like 'UplinkSetCreateExample', 'integration context' do
    ...
    let(:described_class) { OneviewSDK::API600::Synergy::UplinkSet }
  end
end
