...
RSpec.describe klass, integration: true, type: CREATE, sequence: seq(klass) do
  ...
  describe '#create' do
    it 'creates a LIG with interconnect types' do
      ...
      @fc_network = OneviewSDK::API300::Thunderbird::FCNetwork.new($client_300_thunderbird, name: FC_NET_NAME)
      @fc_network.retrieve!

      @fc_lig_uplink_set = OneviewSDK::API300::Thunderbird::LIGUplinkSet.new($client_300_thunderbird, fc_uplink_options)
      @fc_lig_uplink_set.add_network(@fc_network)

      @item = klass.new($client_300_thunderbird, lig_default_options)
    ...
    it 'creates an uplink set and a LIG with internal networks' do
      @ethernet_network = OneviewSDK::API300::Thunderbird::EthernetNetwork.new($client_300_thunderbird, name: ETH_NET_NAME)
      ...
      @item2 = klass.new($client_300_thunderbird, lig_default_options)
  ...
  describe 'getters' do
    it 'gets the default settings' do
      expect { klass.get_default_settings($client_300_thunderbird) }.not_to raise_error
    end

    it 'gets the current settings' do
      item = klass.find_by($client_300_thunderbird, name: LOG_INT_GROUP_NAME).first
