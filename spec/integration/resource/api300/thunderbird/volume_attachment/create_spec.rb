...
RSpec.describe klass, integration: true, type: CREATE, sequence: seq(klass) do
  ...
  describe '::get_all' do
    it 'should get all volume attachments' do
      items = klass.get_all($client_300_thunderbird)
  ...
  describe '::find_by' do
    context 'with no parameters' do
      it 'should get all volume attachments' do
        items = klass.find_by($client_300_thunderbird, {})
        expect(items).not_to be_empty
      end
    end
    ...
    context 'with parameters' do
      it 'should get volume attachments by filter applied' do
        volume = OneviewSDK::API300::Thunderbird::Volume.new($client_300_thunderbird, name: VOLUME_NAME)
        volume.retrieve!

        params = { 'storageVolumeUri' => volume['uri'] }
        items = klass.find_by($client_300_thunderbird, params)
  ...
  describe '::get_extra_unmanaged_volumes' do
    it 'should get the list of extra unmanaged storage volumes' do
      expect { klass.get_extra_unmanaged_volumes($client_300_thunderbird) }.to_not raise_error
    end
  end
  ...
  describe '::remove_extra_unmanaged_volume' do
    xit 'should remove extra presentations from a specific server profile (Skipping this test because occurs error on the http method of ruby)' do
      server_profile = OneviewSDK::API300::Thunderbird::ServerProfile.find_by($client_300_thunderbird, name: SERVER_PROFILE_NAME).first
      expect { klass.remove_extra_unmanaged_volume($client_300_thunderbird, server_profile) }.to_not raise_error
    end
  end

  describe 'retrieve!' do
    it 'should get a volume attachment by uri' do
      item = klass.get_all($client_300_thunderbird).first

      expected_item = klass.new($client_300_thunderbird, uri: item['uri'])
