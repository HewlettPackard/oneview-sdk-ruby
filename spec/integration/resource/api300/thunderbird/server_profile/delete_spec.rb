...
RSpec.describe klass, integration: true, type: DELETE, sequence: rseq(klass) do
  ...
  describe '#delete' do
    it 'deletes all the resources' do
      ...
      names.each do |name|
        item = klass.find_by($client_300_thunderbird, 'name' => name).first
        expect(item).to be
        expect { item.delete }.to_not raise_error
      end
      assoc_vol = OneviewSDK::API300::Thunderbird::Volume.find_by($client_300_thunderbird, 'name' => VOLUME4_NAME).first
