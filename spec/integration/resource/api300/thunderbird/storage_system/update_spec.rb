...
RSpec.describe klass, integration: true, type: UPDATE do
  ...
  describe '#update' do
    it '#updating fc_network unmanaged ports' do
      ...
      storage_system['credentials']['ip_hostname'] = $secrets_thunderbird['storage_system1_ip']
      storage_system.retrieve!

      fc_network = OneviewSDK::API300::C7000::FCNetwork.find_by($client_300_thunderbird, {}).first
      ...
      new_item['credentials']['ip_hostname'] = $secrets_thunderbird['storage_system1_ip']
      new_item.retrieve!
  ...
  describe '#set_refresh_state' do
    it 'Refreshing a storage system' do
      storage_system = klass.find_by($client_300_thunderbird, credentials: { ip_hostname: $secrets_thunderbird['storage_system1_ip'] }).first
  ...
  describe '#like?' do
    ...
    it 'must not compare storage system credentials with password when compares resources' do
      ...
      storage_system['credentials']['ip_hostname'] = $secrets_thunderbird['storage_system1_ip']
      storage_system.retrieve!
      item2 = klass.new($client_300_thunderbird, options)
  ...
  describe '#exists?' do
    it 'finds it by managedDomain' do
      expect(klass.new($client_300_thunderbird, options_2).exists?).to be true
      expect(klass.new($client_300_thunderbird, managedDomain: 'fake', credentials: { ip_hostname: 'fake' }).exists?).to be false
    end

    it 'finds it by credentials' do
      expect(klass.new($client_300_thunderbird, credentials: { ip_hostname: $secrets_thunderbird['storage_system1_ip'] }).exists?).to be true
      expect(klass.new($client_300_thunderbird, credentials: { ip_hostname: 'fake' }).exists?).to be false
    end
    ...
    it 'raises an exception when ip_hostname is not passed' do
      item = klass.new($client_300_thunderbird, credentials: {})
      expect { item.exists? }.to raise_error(OneviewSDK::IncompleteResource)
    end
  end
end
