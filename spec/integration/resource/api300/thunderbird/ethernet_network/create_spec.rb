...
RSpec.describe klass, integration: true, type: CREATE, sequence: seq(klass) do
  ...
  describe '#create' do
    it 'can create resources' do
      item = OneviewSDK::API300::Thunderbird::EthernetNetwork.from_file($client, file_path)
      item.create
    ...
    it 'bulk create ethernet resources' do
      ...
      bulk_networks = OneviewSDK::API300::Thunderbird::EthernetNetwork.bulk_create($client, bulk_options)
  ...
  describe '#retrieve!' do
    it 'retrieves the resource' do
      item = OneviewSDK::API300::Thunderbird::EthernetNetwork.new($client, name: ETH_NET_NAME)
      item.retrieve!
      expect(item[:name]).to eq(ETH_NET_NAME)
      expect(item[:ethernetNetworkType]).to eq('Tagged')
      expect(item[:vlanId]).to eq(1001)
  ...
  describe '#find_by' do
    it 'returns all resources when the hash is empty' do
      names = OneviewSDK::API300::Thunderbird::EthernetNetwork.find_by($client, {}).map { |item| item[:name] }
    ...
    it 'finds networks by multiple attributes' do
      ...
      names = OneviewSDK::API300::Thunderbird::EthernetNetwork.find_by($client, attrs).map { |item| item[:name] }
      expect(names).to include(ETH_NET_NAME)
    end
  end
end
