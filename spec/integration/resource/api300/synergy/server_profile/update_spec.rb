...
RSpec.describe klass, integration: true, type: UPDATE do
  ...
  before :all do
    ...
    @item3 = klass.new($client_300_synergy, name: SERVER_PROFILE2_NAME)
    @item3.retrieve!
  end
  ...
  describe '#self.get_available_networks' do
    it 'retrieves available networks without errors' do
      ...
      expect { klass.get_available_networks($client_300_synergy, query_options) }.to_not raise_error
    end
  end

  describe '#self.get_available_servers' do
    it 'retrieves available servers without errors' do
      expect { klass.get_available_servers($client_300_synergy) }.to_not raise_error
    end
  end
  ...
  describe '#self.get_available_storage_system' do
    it 'retrieves available storage system without errors. FAIL: Bug in OneView/Documentation' do
      ...
      expect { klass.get_available_storage_system($client_300_synergy, query_options) }.to_not raise_error
    end
  end
  ...
  describe '#self.get_available_storage_systems' do
    it 'retrieves available storage systems without errors' do
      ...
      expect { klass.get_available_storage_systems($client_300_synergy, query_options) }.to_not raise_error
    end
  end

  describe '#self.get_available_targets' do
    it 'retrieves available targets without errors' do
      expect { klass.get_available_targets($client_300_synergy) }.to_not raise_error
    end
  end
  ...
  describe '#self.get_profile_ports' do
    it 'retrieves profile ports without errors' do
      ...
      expect { klass.get_profile_ports($client_300_synergy, query_options) }.to_not raise_error
    end
  end
  ...
  describe '#get_sas_logical_jbods' do
    it 'returns all SAS Logical JBODs' do
      expect { klass.get_sas_logical_jbods($client_300_synergy) }.not_to raise_error
    end
  end

  describe '#get_sas_logical_jbod_attachments' do
    it 'retrieves all attachments' do
      expect { klass.get_sas_logical_jbod_attachments($client_300_synergy) }.not_to raise_error
    end
  end
end
