...
RSpec.describe klass, integration: true, type: CREATE, sequence: seq(klass) do
  ...
  before :each do
    @enclosure_group = extra_klass_1.find_by($client_300_synergy, name: ENC_GROUP2_NAME).first
    @enclosure1 = extra_klass_2.find_by($client_300_synergy, name: ENCL2_NAME).first
    @enclosure2 = extra_klass_2.find_by($client_300_synergy, name: ENCL3_NAME).first
    @enclosure3 = extra_klass_2.find_by($client_300_synergy, name: ENCL4_NAME).first
  end
