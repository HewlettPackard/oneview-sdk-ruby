...
RSpec.describe klass, integration: true, type: CREATE, sequence: seq(klass) do
  ...
  describe '#retrieve!' do
    it 'retrieves the resource' do
      storage_system_ref = storage_system_klass.new($client_300, storage_system_data)
  ...
  describe '#exists?' do
    it 'returns true if storage pool exists' do
      storage_system = storage_system_klass.new($client_300, storage_system_data)
      storage_system.retrieve!
      item = klass.new($client_300, name: STORAGE_POOL_NAME, storageSystemUri: storage_system['uri'])
    ...
    it 'returns false if storage pool not exists' do
      storage_system = storage_system_klass.new($client_300, storage_system_data)
      storage_system.retrieve!
      item = klass.new($client_300, name: 'some unknown nama', storageSystemUri: storage_system['uri'])
    ...
    it 'should throw incomplete resource exception if name and uri or storageSystemUri are unknown' do
      storage_system = storage_system_klass.new($client_300, storage_system_data)
      storage_system.retrieve!
      item_without_name_and_uri = klass.new($client_300, storageSystemUri: storage_system['uri'])
      item_without_storage_system_uri = klass.new($client_300, name: 'some unknown nama')
