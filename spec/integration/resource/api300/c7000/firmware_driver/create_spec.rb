...
RSpec.describe klass, integration: true, type: CREATE, sequence: seq(klass) do
  ...
  describe '#create' do
    it 'Create custom spp (it will fail if the applicance does not have a valid spp and hotfix)' do
      spp = klass.find_by($client_300, state: 'Created', bundleType: 'SPP').first
      hotfix = klass.find_by($client_300, state: 'Created', bundleType: 'Hotfix').first
      expect(spp['uri']).to be
      expect(hotfix['uri']).to be
      custom = klass.new($client_300)
