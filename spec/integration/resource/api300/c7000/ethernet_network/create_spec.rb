...
RSpec.describe klass, integration: true, type: CREATE, sequence: seq(klass) do
  ...
  describe '#create' do
    it 'can create resources' do
      item = OneviewSDK::API300::C7000::EthernetNetwork.from_file($client, file_path)
      item.create
    ...
    it 'bulk create ethernet resources' do
      ...
      bulk_networks = OneviewSDK::API300::C7000::EthernetNetwork.bulk_create($client, bulk_options)
  ...
  describe '#find_by' do
    it 'returns all resources when the hash is empty' do
      names = OneviewSDK::API300::C7000::EthernetNetwork.find_by($client, {}).map { |item| item[:name] }
    ...
    it 'finds networks by multiple attributes' do
      ...
      names = OneviewSDK::API300::C7000::EthernetNetwork.find_by($client, attrs).map { |item| item[:name] }
      expect(names).to include(ETH_NET_NAME)
    end
  end
end
