...
RSpec.describe klass, integration: true, type: UPDATE do
  ...
  describe '#set_refresh_state' do
    it 'Refreshing a storage system' do
      storage_system = klass.find_by($client_300, credentials: { ip_hostname: $secrets['storage_system1_ip'] }).first
  ...
  describe '#like?' do
    ...
    it 'must not compare storage system credentials with password when compares resources' do
      ...
      storage_system['credentials']['ip_hostname'] = $secrets['storage_system1_ip']
      storage_system.retrieve!
      item2 = klass.new($client_300, options)
  ...
  describe '#exists?' do
    it 'finds it by managedDomain' do
      expect(klass.new($client_300, options_2).exists?).to be true
      expect(klass.new($client_300, managedDomain: 'fake', credentials: { ip_hostname: 'fake' }).exists?).to be false
    end

    it 'finds it by credentials' do
      expect(klass.new($client_300, credentials: { ip_hostname: $secrets['storage_system1_ip'] }).exists?).to be true
      expect(klass.new($client_300, credentials: { ip_hostname: 'fake' }).exists?).to be false
    end
    ...
    it 'raises an exception if no identifiers are given' do
      item = klass.new($client_300, credentials: {})
      expect { item.exists? }.to raise_error(OneviewSDK::IncompleteResource)
    end
  end
end
