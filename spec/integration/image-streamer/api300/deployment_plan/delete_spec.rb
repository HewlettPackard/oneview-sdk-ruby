...
RSpec.describe klass, integration_i3s: true, type: DELETE, sequence: i3s_rseq(klass) do
  ...
  describe '#delete' do
    it 'removes all deployment plans' do
      item = klass.find_by($client_i3s_300, name: DEPLOYMENT_PLAN1_NAME).first
      item2 = klass.find_by($client_i3s_300, name: DEPLOYMENT_PLAN2_NAME).first
      item3 = klass.find_by($client_i3s_300, name: DEPLOYMENT_PLAN3_NAME).first
      expect(item).to be
      expect(item2).to be
      expect(item3).to be
