...
RSpec.describe klass, integration_i3s: true, type: CREATE, sequence: i3s_seq(klass) do
  ...
  describe '#create' do
    ...
    it 'creates a build plan with build step' do
      plan_script = OneviewSDK::ImageStreamer::API600::PlanScript.find_by($client_i3s_600, name: PLAN_SCRIPT1_NAME).first
    ...
    it 'creates a build plan with build step and custom attributes' do
      plan_script = OneviewSDK::ImageStreamer::API600::PlanScript.find_by($client_i3s_600, name: PLAN_SCRIPT2_NAME).first
