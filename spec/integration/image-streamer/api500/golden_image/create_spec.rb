...
RSpec.describe klass, integration_i3s: true, type: CREATE, sequence: i3s_seq(klass) do
  ...
  describe '#create' do
    it 'creates a golden image' do
      os_volume = OneviewSDK::ImageStreamer::API500::OSVolume.find_by($client_i3s_500, {}).first
      build_plan = OneviewSDK::ImageStreamer::API500::BuildPlan.find_by($client_i3s_500, oeBuildPlanType: 'capture').first
  ...
  describe '#add' do
    it 'adding a golden image' do
      ...
      item = nil
      expect { item = klass.add($client_i3s_500, golden_image_upload_path, options) }.not_to raise_error
      item.retrieve!
      expect(item['uri']).to be
      expect(item['name']).to eq(options[:name])
      expect(item['description']).to eq(options[:description])
