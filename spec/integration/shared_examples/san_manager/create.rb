...
RSpec.shared_examples 'SANManagerCreateExample' do |context_name|
  ...
  describe '#add' do
    it 'can add resources' do
      item = OneviewSDK::SANManager.new(current_client)
      item['providerDisplayName'] = provider_name
  ...
  describe '#self.get_default_connection_info' do
    it 'Retrieve connection info for provider' do
      expect { OneviewSDK::SANManager.get_default_connection_info(current_client, SAN_PROVIDER1_NAME) }.to_not raise_error
    end
  end
