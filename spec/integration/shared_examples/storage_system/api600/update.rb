RSpec.shared_examples 'StorageSystemUpdateExample API600' do
  describe '#update' do
    it 'should update port to have a network correctly' do
      item = described_class.new($client_600, item_attributes)
      item.retrieve!
      fc_network = OneviewSDK::API600::C7000::FCNetwork.find_by($client_600, name: FC_NET_NAME).first
  ...
  describe '#retrieve!' do
    it 'should retrieve using wwn' do
      ...
      item_retrieved = described_class.new($client_600, 'deviceSpecificAttributes' => { 'wwn' => wwn })
    ...
    it 'should retrieve using serialNumber' do
      ...
      item_retrieved = described_class.new($client_600, 'deviceSpecificAttributes' => { 'serialNumber' => serial_number })
      expect(item_retrieved.retrieve!).to eq(true)
      expect(item_retrieved['uri']).to eq(item['uri'])
    ...
    it 'should retrieve using hostname' do
      ...
      item_retrieved = described_class.new($client_600, 'hostname' => item['hostname'])
      expect(item_retrieved.retrieve!).to eq(true)
      expect(item_retrieved['uri']).to eq(item['uri'])
    end
    ...
    context 'when value for retrieve is wrong' do
      it 'should return false and not retrieve data' do
        item_retrieved = described_class.new($client_600, 'deviceSpecificAttributes' => { 'wwn' => 'fake' })
        expect(item_retrieved.retrieve!).to eq(false)

        item_retrieved = described_class.new($client_600, 'deviceSpecificAttributes' => { 'serialNumber' => 'fake' })
        expect(item_retrieved.retrieve!).to eq(false)

        item_retrieved = described_class.new($client_600, 'hostname' => 'fake')
        expect(item_retrieved.retrieve!).to eq(false)
      end
    end
  end
...
RSpec.shared_examples 'StorageSystemUpdateExample StoreServ API600' do
  describe '#update' do
  ...
  include_examples 'StorageSystemUpdateExample API600'
end
