...
RSpec.shared_examples 'LIGC7000DeleteExample' do |context_name|
  ...
  subject(:item_4) { described_class.find_by(current_client, name: LOG_INT_GROUP4_NAME).first }
  ...
  describe '#delete' do
    it 'removes all the Logical Interconnect groups' do
      ...
      expect(item_4.retrieve!).to eq(false)
    end
  end
end
