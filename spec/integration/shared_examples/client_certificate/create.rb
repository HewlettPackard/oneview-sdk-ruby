...
RSpec.shared_examples 'ClientCertificateCreateExample' do |context_name|
  ...
  describe '#get_all' do
    ...
    it 'should get all certificates' do
      expect(described_class.get_all(current_client, header)).not_to be_empty
    end
  end

  describe '#create' do
    it 'should raise MethodUnavailable error' do
      expect { item.create(header) }.to raise_error(OneviewSDK::MethodUnavailable)
    end
  end
  ...
  describe '#import' do
    it 'should import the given SSL certificate' do
      ...
      expect { item.import(header) }.not_to raise_error
  ...
  describe '#refresh' do
    it 'should update the current object using the data that exists on OneView' do
      expect { item.refresh(header) }.not_to raise_error
