...
RSpec.shared_examples 'FirmwareDriverDeleteExample' do |context_name|
  ...
  describe '#remove' do
    it 'deletes the resource' do
      item = described_class.new(current_client, name: FIRMWARE_DRIVER1_NAME)
      item.retrieve!
      expect { item.remove }.not_to raise_error
    end

    it 'deletes other drivers' do
      described_class.find_by(current_client, {}).each do |driver|
