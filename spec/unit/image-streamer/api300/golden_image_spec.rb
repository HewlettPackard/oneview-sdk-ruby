...
RSpec.describe klass do
  ...
  describe '#create' do
    it 'generates exception when setting an os volume without uri' do
      item = klass.new(@client_i3s_300)
      expect { item.set_os_volume(OneviewSDK::ImageStreamer::API300::OsVolumes.new(@client_i3s_300)) }
        .to raise_error(OneviewSDK::IncompleteResource, /Please set/)
    end

    it 'sets as os volume successfully' do
    ...
    it 'generates exception when setting a build plan without uri' do
      item = klass.new(@client_i3s_300)
      expect { item.set_os_volume(OneviewSDK::ImageStreamer::API300::BuildPlan.new(@client_i3s_300)) }
        .to raise_error(OneviewSDK::IncompleteResource, /Please set/)
    end
