  ...
  describe '#delete_from_file' do
    ...
    context 'with valid options' do
      before :each do
        ...
        allow_any_instance_of(OneviewSDK::BaseResource).to receive(:retrieve!).and_return(true)
        allow_any_instance_of(OneviewSDK::BaseResource).to receive(:refresh).and_return(true)
        allow_any_instance_of(HighLine).to receive(:agree).and_return(true)
      end

      it 'deletes a valid resource by name' do
        allow_any_instance_of(OneviewSDK::BaseResource).to receive(:refresh).and_return(false)
      ...
      it 'deletes a valid resource by uri' do
        allow_any_instance_of(OneviewSDK::BaseResource).to receive(:retrieve!).and_return(false)
        expect_any_instance_of(HighLine).to receive(:agree)
