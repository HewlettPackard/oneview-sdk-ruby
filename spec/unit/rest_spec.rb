...
RSpec.describe OneviewSDK::Client do
  ...
  describe '#rest_api' do
    ...
    it 'requires a path' do
      expect { @client_600.rest_api(:get, nil) }.to raise_error(OneviewSDK::InvalidRequest, /Must specify path/)
    end

    it 'logs the request type and path (debug level)' do
      @client_600.logger.level = @client_600.logger.class.const_get('DEBUG')
      %w[get post put patch delete].each do |type|
        expect { @client_600.rest_api(type, path) }
          .to output(/Making :#{type} rest call to #{@client_600.url + path}/).to_stdout_from_any_process
      end
    end
    ...
    it 'raises an error when the ssl validation fails' do
      ...
      expect(@client_600.logger).to receive(:error).with(/SSL verification failed/)
      expect { @client_600.rest_api(:get, path) }.to raise_error(OpenSSL::SSL::SSLError)
    end
    ...
    it 'supports following redirects' do
      ...
      r = @client_600.rest_api(:get, path)
    ...
    it 'follows a limited number of redirects' do
      ...
      expect(@client_600).to receive(:rest_api).exactly(4).times.and_call_original
      r = @client_600.rest_api(:get, path)
    ...
    it 'only follows redirects if there is a location header' do
      ...
      allow(response.class).to receive(:<=).with(Net::HTTPRedirection).and_return(true)
      allow_any_instance_of(Net::HTTP).to receive(:request).and_return(response)
      expect(@client_600).to receive(:rest_api).once.and_call_original
      @client_600.rest_api(:get, path)
    end
  end

  describe '#rest_get' do
    it 'calls rest_api' do
      expect(@client_600).to receive(:rest_api).with(:get, path, { some_header: data }, @client_600.api_version)
      @client_600.rest_get(path, { some_header: data }, @client_600.api_version)
    end

    it 'has default options and api_ver' do
      expect(@client_600).to receive(:rest_api).with(:get, path, {}, @client_600.api_version)
      @client_600.rest_get(path)
    end
  end

  describe '#rest_post' do
    it 'calls rest_api' do
      expect(@client_600).to receive(:rest_api).with(:post, path, { body: data }, @client_600.api_version)
      @client_600.rest_post(path, { body: data }, @client_600.api_version)
    end

    it 'has default options and api_ver' do
      expect(@client_600).to receive(:rest_api).with(:post, path, {}, @client_600.api_version)
      @client_600.rest_post(path)
    end
  end

  describe '#rest_put' do
    it 'calls rest_api' do
      expect(@client_600).to receive(:rest_api).with(:put, path, {}, @client_600.api_version)
      @client_600.rest_put(path, {}, @client_600.api_version)
    end

    it 'has default options and api_ver' do
      expect(@client_600).to receive(:rest_api).with(:put, path, {}, @client_600.api_version)
      @client_600.rest_put(path)
    end
  end

  describe '#rest_patch' do
    it 'calls rest_api' do
      expect(@client_600).to receive(:rest_api).with(:patch, path, {}, @client_600.api_version)
      @client_600.rest_patch(path, {}, @client_600.api_version)
    end

    it 'has default options and api_ver' do
      expect(@client_600).to receive(:rest_api).with(:patch, path, {}, @client_600.api_version)
      @client_600.rest_patch(path)
    end
  end

  describe '#rest_delete' do
    it 'calls rest_api' do
      expect(@client_600).to receive(:rest_api).with(:delete, path, {}, @client_600.api_version)
      @client_600.rest_delete(path, {}, @client_600.api_version)
    end

    it 'has default options and api_ver' do
      expect(@client_600).to receive(:rest_api).with(:delete, path, {}, @client_600.api_version)
      @client_600.rest_delete(path)
    end
  end

  describe '#response_handler' do
    it 'returns the JSON-parsed body for 200 status' do
      expect(@client_600.response_handler(FakeResponse.new(data))).to eq(data)
    end

    it 'returns the JSON-parsed body for 201 status' do
      expect(@client_600.response_handler(FakeResponse.new(data, 201))).to eq(data)
    end
    ...
    it 'waits on the task completion for 202 status' do
      ...
      expect(@client_600).to receive(:wait_for).with('/rest/task/fake').and_return(wait_response)

      expect(@client_600).to receive(:rest_get).with(data['uri']).and_return(FakeResponse.new(data))
      expect(@client_600.response_handler(initial_response)).to eq(data)
    end

    it 'allows you to set wait_for_task to false' do
      response = FakeResponse.new(data, 202, 'location' => '/rest/task/fake')
      expect(@client_600.response_handler(response, false)).to eq(data)
    end

    it 'returns an empty hash for 204 status' do
      expect(@client_600.response_handler(FakeResponse.new({}, 204))).to eq({})
    end

    it 'raises an error for 400 status' do
      resp = FakeResponse.new({ message: 'Blah' }, 400)
      expect { @client_600.response_handler(resp) }.to raise_error { |e|
    ...
    it 'raises an error for undefined status codes' do
      [0, 19, 199, 203, 399, 402, 500].each do |status|
        ...
        expect { @client_600.response_handler(resp) }.to raise_error(OneviewSDK::RequestError, /#{status}.*Blah/)
      end
    end
  end
  ...
  describe '#build_request' do
    before :each do
      @uri = URI.parse(Addressable::URI.escape(@client_600.url + path))
      @options = {
        'X-API-Version' => @client_600.api_version,
        'auth' => @client_600.token
      }
    end

    it 'fails when an invalid request type is given' do
      expect { @client_600.send(:build_request, :fake, @uri, {}, @client_600.api_version) }
        .to raise_error(OneviewSDK::InvalidRequest, /Invalid rest method/)
    end

    context 'default header values' do
      before :each do
        @req = @client_600.send(:build_request, :get, @uri, {}, @client_600.api_version)
      end

      it 'sets the X-API-Version' do
        expect(@req['x-api-version']).to eq(@client_600.api_version.to_s)
      end

      it 'sets the auth token' do
        expect(@req['auth']).to eq(@client_600.token)
      end
    ...
    it 'allows deletion of default headers' do
      ...
      req = @client_600.send(:build_request, :get, @uri, options, @client_600.api_version)
    ...
    it 'logs the request options (debug level)' do
      def_options = { 'X-API-Version' => @client_600.api_version, 'auth' => 'secretToken', 'Content-Type' => 'application/json' }
      @client_600.logger.level = @client_600.logger.class.const_get('DEBUG')
      expect { @client_600.send(:build_request, :get, @uri, {}, @client_600.api_version) }
  ...
  describe 'upload_file' do
    context 'when file does not exist' do
      it 'should raise exception' do
        expect { @client_600.upload_file('file.zip', '/uri') }.to raise_error(OneviewSDK::NotFound, //)
      end
    end
    ...
    it 'should use default parameter values' do
      ...
      allow(@client_600).to receive(:response_handler).and_return(FakeResponse.new)
      @client_600.upload_file('file.zip', '/uri-file-upload')
    ...
    it 'should use value of passed as parameter' do
      ...
      @client_600.upload_file('file.zip', '/uri-file-upload', { 'body' => { 'name' => 'TestName' } }, 600)

      expected_options = { 'Content-Type' => 'multipart/form-data', 'X-Api-Version' => @client_600.api_version.to_s, 'auth' => @client_600.token }
    ...
    it 'upload a file' do
      ...
      allow(File).to receive(:open).with('file.zip').and_yield('FAKE FILE CONTENT')
      allow(UploadIO).to receive(:new).and_return('FAKE FILE CONTENT')
      result = @client_600.upload_file('file.zip', 'rest/fake/1', options)
    ...
    it 'raises an exception when timeout expire' do
      ...
      allow(UploadIO).to receive(:new).and_return('FAKE FILE CONTENT')
      expect { @client_600.upload_file('file.tar', 'rest/fake/1') }.to raise_error(/The connection was closed/)
    end
  end
  ...
  describe '::download_file' do
    it 'should download file correctly' do
      ...
      expect(@client_600).to receive(:build_request)
      expect(@client_600).not_to receive(:response_handler)

      result = @client_600.download_file('/file-download-uri', 'file.zip')
    ...
    context 'when http response has a status error' do
      it 'should call response_handler from client class' do
        ...
        expect { @client_600.download_file('/file-download-uri', 'file.zip') }.to raise_error(OneviewSDK::BadRequest)
      end
    end
  end
  ...
  describe '#build_http_object' do
    context 'with ssl enabled and timeout variable defined' do
      it 'should create a http object with valid data' do
        ...
        @client_600.ssl_enabled = true
        @client_600.cert_store = 'some_certificate'
        @client_600.timeout = 300

        http = @client_600.send(:build_http_object, uri)
