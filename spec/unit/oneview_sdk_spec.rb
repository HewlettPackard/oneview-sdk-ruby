...
RSpec.describe OneviewSDK do
  ...
  it 'has a list of supported api versions' do
    ...
    [200, 300, 500, 600, 800, 1000, 1200, 1600, 1800, 2000, 2200, 2400].each { |v| expect(versions).to include(v) }
  end

  it 'returns a valid API version' do
    %w[API200 API300 API500 API600 API800 API1000 API1200 API1600 API1800 API2000 API2200 API2400]
  ...
  it 'raises an error when an invalid API300 version is called' do
    expect { OneviewSDK::API999 }.to raise_error(NameError,
                                                 'The API999 method or resource does not exist for OneView API version 2400.')
  end
