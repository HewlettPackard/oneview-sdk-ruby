...
RSpec.describe OneviewSDK::ResourceHelper do
  ...
  subject(:klass) { SomeIncluder.new(@client_600) }

  describe '#patch' do
    it 'requires a uri' do
      item = SomeIncluder.new(@client_600)
      expect { item.patch('replace', '/path', 'any') }.to raise_error(OneviewSDK::IncompleteResource, /Please set uri/)
    end

    it 'executing a patch' do
      item = SomeIncluder.new(@client_600, uri: '/rest/fake', eTag: 'anyTag')
      options = {
        'If-Match' => item['eTag'],
        'body' => [{ op: 'replace', path: '/path', value: 'val' }]
      }
      expect(@client_600).to receive(:rest_patch).with(item['uri'], options, item.api_version).and_return(FakeResponse.new)
  ...
  describe '#configuration' do
    it 'requires a uri' do
      item = SomeIncluderConfigurationOperation.new(@client_600)
      expect { item.configuration }.to raise_error(OneviewSDK::IncompleteResource, /Please set uri/)
    end

    it 'applying configuration' do
      item = SomeIncluderConfigurationOperation.new(@client_600, uri: '/rest/fake/1')
      allow_any_instance_of(SomeIncluderConfigurationOperation).to receive(:retrieve!).and_return(true)
      allow(@client_600).to receive(:response_handler)
      expect(@client_600).to receive(:rest_put).with('/rest/fake/1/configuration', {}, item.api_version).and_return(FakeResponse.new)
