...
RSpec.describe OneviewSDK::SCMB do
  ...
  describe '::new_connection' do
    ...
    it 'creates a Bunny connection with the correct defaults' do
      ...
      expect(described_class.new_connection(@client_300)).to eq(con)
    end

    it 'allows you to override the defaults' do
      allow(Bunny).to receive(:new) do |opts|
        expect(opts[:host]).to eq(URI.parse(@client_300.url).host) # Doesn't actually override the value
      ...
      described_class.new_connection(@client_300, override_opts)
    end
  end

  describe '::get_or_create_keypair' do
    it 'gets the default rabbitmq keypair' do
      expect(@client_300).to receive(:rest_get).with(keypair_uri).and_return(FakeResponse.new(keypair))
