...
RSpec.describe OneviewSDK::API300::FirmwareBundle do
  ...
  describe '#add' do
    ...
    it 'returns a FirmwareDriver resource' do
      expect(@client_300).to receive(:rest_post).with(
        OneviewSDK::API300::FirmwareBundle::BASE_URI,
        hash_including(
          'body' => %r{Content-Type: application/octet-stream; Content-Transfer-Encoding: binary\r\n\r\nFAKE FILE CONTENT\r\n--},
          'uploadfilename' => 'file.tar',
          'Content-Type' => "multipart/form-data; boundary=#{OneviewSDK::API300::FirmwareBundle::BOUNDARY}"
        )
      ).and_return(FakeResponse.new({}))
      expect(File).to receive('file?').and_return(true)
      allow(IO).to receive(:binread).and_return('FAKE FILE CONTENT')
      OneviewSDK::API300::FirmwareBundle.add(@client_300, 'file.tar')
    end
  end
end
