...
RSpec.describe OneviewSDK::API300::ServerProfileTemplate do
  ...
  before(:each) do
    @item = OneviewSDK::API300::ServerProfileTemplate.new(@client_300, name: 'server_profile_template')
  end
  ...
  describe '#set_enclosure_group' do
    before :each do
      @enclosure_group = OneviewSDK::API300::EnclosureGroup.new(@client_300, name: 'enclosure_group')
  ...
  describe '#set_firmware_driver' do
    before :each do
      ...
      @firmware = OneviewSDK::API300::FirmwareDriver.new(@client_300, name: 'unit_firmware_driver', uri: @firmware_uri)
    end
  ...
  describe '#available_hardware' do
    it 'requires the serverHardwareTypeUri value to be set' do
      expect { OneviewSDK::API300::ServerProfileTemplate.new(@client_300).get_available_hardware }
    ...
    it 'requires the enclosureGroupUri value to be set' do
      expect { OneviewSDK::API300::ServerProfileTemplate.new(@client_300, serverHardwareTypeUri: '/rest/fake').get_available_hardware }
  ...
  describe '#new_profile' do
    it 'returns a profile' do
      ...
      template = OneviewSDK::API300::ServerProfileTemplate.new(@client_300, name: 'unit_server_profile_template')
    ...
    it 'can set the name of a new profile' do
      allow_any_instance_of(OneviewSDK::Client).to receive(:rest_get).and_return(FakeResponse.new(name: 'NewProfile'))
      template = OneviewSDK::API300::ServerProfileTemplate.new(@client_300, uri: '/rest/server-profile-templates/fake')
  ...
  describe 'Volume attachment operations' do
    ...
    it 'can call #create_volume_with_attachment and generate the data required for a new Volume with attachment' do
      storage_pool = OneviewSDK::API300::StoragePool.new(@client_300, uri: 'fake/storage-pool')
      volume_options = {
        name: 'TestVolume',
