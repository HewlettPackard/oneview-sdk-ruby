...
RSpec.describe OneviewSDK::API300::Enclosure do
  ...
  describe '#initialize' do
    context 'OneView 2.0' do
      it 'sets the defaults correctly' do
        enclosure = OneviewSDK::API300::Enclosure.new(@client_300)
  ...
  describe '#add' do
    context 'with valid data' do
      before :each do
        ...
        @enclosure = OneviewSDK::API300::Enclosure.new(@client_300, @data)
      end
  ...
  describe '#update' do
    before :each do
      @item  = OneviewSDK::API300::Enclosure.new(@client_300, name: 'E1', rackName: 'R1', uri: '/rest/fake')
      @item2 = OneviewSDK::API300::Enclosure.new(@client_300, name: 'E2', rackName: 'R1', uri: '/rest/fake2')
      @item3 = OneviewSDK::API300::Enclosure.new(@client_300, name: 'E1', rackName: 'R2', uri: '/rest/fake2')
    end

    it 'requires a uri' do
      expect { OneviewSDK::API300::Enclosure.new(@client_300).update }.to raise_error(OneviewSDK::IncompleteResource, /Please set uri/)
    end

    it 'does not send a PATCH request if the name and rackName are the same' do
      expect(OneviewSDK::API300::Enclosure).to receive(:find_by).with(@client_300, uri: @item['uri']).and_return([@item])
      expect(@client_300).to_not receive(:rest_patch)
      @item.update
    end

    it 'updates the server name with the local name' do
      expect(OneviewSDK::API300::Enclosure).to receive(:find_by).with(@client_300, uri: @item['uri']).and_return([@item2])
    ...
    it 'updates the server rackName with the local rackName' do
      expect(OneviewSDK::API300::Enclosure).to receive(:find_by).with(@client_300, uri: @item['uri']).and_return([@item3])
      expect(@client_300).to receive(:rest_patch)
  ...
  describe '#remove' do
    it 'removes enclosure' do
      item = OneviewSDK::API300::Enclosure.new(@client_300, name: 'E1', rackName: 'R1', uri: '/rest/enclosures/encl1')
  ...
  describe '#set_refresh_state' do
    ...
    it 'does a PUT to /refreshState' do
      item = OneviewSDK::API300::Enclosure.new(@client_300, uri: '/rest/fake', refreshState: 'NotRefreshing')
  ...
  describe '#script' do
    it 'requires a uri' do
      expect { OneviewSDK::API300::Enclosure.new(@client_300).script }.to raise_error(OneviewSDK::IncompleteResource, /Please set uri/)
    end
  ...
  describe '#updateAttribute' do
    it 'requires a uri' do
      expect { OneviewSDK::API300::Enclosure.new(@client_300).patch(:op, :path, :val) }
  ...
  describe '#convert_time' do
    before :each do
      @item = OneviewSDK::API300::Enclosure.new(@client_300)
      @t = Time.now
      @d = Date.today
    end
