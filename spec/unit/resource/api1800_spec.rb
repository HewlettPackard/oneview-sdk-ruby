...
RSpec.describe OneviewSDK::API1800 do
  ...
  it 'returns a valid API1800 variant' do
    %w[C7000 Synergy].each { |v| expect { OneviewSDK::API1800.const_get(v) }.not_to raise_error }
  end

  it 'raises an error when an invalid API1800 variant is called' do
    expect { OneviewSDK::API1800::C6000 }
      .to raise_error(NameError, 'The C6000 method or resource does not exist for OneView API1800 variant C7000.')
  end
  ...
  describe '#variant=' do
    it 'sets the current variant' do
      ...
      expect(OneviewSDK::API1800.variant).to eq('Synergy')
      expect(OneviewSDK::API1800.variant_updated?).to eq(true)
    end
  end
end
