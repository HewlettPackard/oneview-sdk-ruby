...
# Context for API1800 integration testing:
RSpec.shared_context 'integration api1800 context', a: :b do
  before :all do
    integration_context
    $client_1800 ||= OneviewSDK::Client.new($config.merge(api_version: 1800))
    $client_1800_synergy ||= OneviewSDK::Client.new($config_synergy.merge(api_version: 1800))
  end
end

# Context for API2000 integration testing:
RSpec.shared_context 'integration api2000 context', a: :b do
  before :all do
    integration_context
    $client_2000 ||= OneviewSDK::Client.new($config.merge(api_version: 2000))
    $client_2000_synergy ||= OneviewSDK::Client.new($config_synergy.merge(api_version: 2000))
  end
end

# Context for API2200 integration testing:
RSpec.shared_context 'integration api2200 context', a: :b do
  before :all do
    integration_context
    $client_2200 ||= OneviewSDK::Client.new($config.merge(api_version: 2200))
    $client_2200_synergy ||= OneviewSDK::Client.new($config_synergy.merge(api_version: 2200))
  end
end

# Context for API2400 integration testing:
RSpec.shared_context 'integration api2400 context', a: :b do
  before :all do
    integration_context
    $client_2400 ||= OneviewSDK::Client.new($config.merge(api_version: 2400))
    $client_2400_synergy ||= OneviewSDK::Client.new($config_synergy.merge(api_version: 2400))
  end
end

# Context for API2600 integration testing:
RSpec.shared_context 'integration api2600 context', a: :b do
  before :all do
    integration_context
    $client_2600 ||= OneviewSDK::Client.new($config.merge(api_version: 2600))
    $client_2600_synergy ||= OneviewSDK::Client.new($config_synergy.merge(api_version: 2600))
  end
end

# Context for API2800 integration testing:
RSpec.shared_context 'integration api2800 context', a: :b do
  before :all do
    integration_context
    $client_2800 ||= OneviewSDK::Client.new($config.merge(api_version: 2800))
    $client_2800_synergy ||= OneviewSDK::Client.new($config_synergy.merge(api_version: 2800))
  end
end

# Context for API3000 integration testing:
RSpec.shared_context 'integration api3000 context', a: :b do
  before :all do
    integration_context
    $client_3000 ||= OneviewSDK::Client.new($config.merge(api_version: 3000))
    $client_3000_synergy ||= OneviewSDK::Client.new($config_synergy.merge(api_version: 3000))
  end
end
...
RSpec.shared_context 'system api1800 context', a: :b do
  before(:each) do
    load_system_properties
    generate_clients(1800)
  end
end

RSpec.shared_context 'system api2000 context', a: :b do
  before(:each) do
    load_system_properties
    generate_clients(2000)
  end
end

RSpec.shared_context 'system api2200 context', a: :b do
  before(:each) do
    load_system_properties
    generate_clients(2200)
  end
end

RSpec.shared_context 'system api2400 context', a: :b do
  before(:each) do
    load_system_properties
    generate_clients(2400)
  end
end

RSpec.shared_context 'system api2600 context', a: :b do
  before(:each) do
    load_system_properties
    generate_clients(2600)
  end
end

RSpec.shared_context 'system api2800 context', a: :b do
  before(:each) do
    load_system_properties
    generate_clients(2800)
  end
end

RSpec.shared_context 'system api3000 context', a: :b do
  before(:each) do
    load_system_properties
    generate_clients(3000)
  end
end
...
def generate_clients(api_version)
  ...
  when 1800
    $client_1800 ||= OneviewSDK::Client.new($config.merge(api_version: api_version))
    $client_1800_synergy ||= OneviewSDK::Client.new($config_synergy.merge(api_version: api_version))
  when 2000
    $client_2000 ||= OneviewSDK::Client.new($config.merge(api_version: api_version))
    $client_2000_synergy ||= OneviewSDK::Client.new($config_synergy.merge(api_version: api_version))
  when 2200
    $client_2200 ||= OneviewSDK::Client.new($config.merge(api_version: api_version))
    $client_2200_synergy ||= OneviewSDK::Client.new($config_synergy.merge(api_version: api_version))
  when 2400
    $client_2400 ||= OneviewSDK::Client.new($config.merge(api_version: api_version))
    $client_2400_synergy ||= OneviewSDK::Client.new($config_synergy.merge(api_version: api_version))
  when 2600
    $client_2600 ||= OneviewSDK::Client.new($config.merge(api_version: api_version))
    $client_2600_synergy ||= OneviewSDK::Client.new($config_synergy.merge(api_version: api_version))
  when 2800
    $client_2800 ||= OneviewSDK::Client.new($config.merge(api_version: api_version))
    $client_2800_synergy ||= OneviewSDK::Client.new($config_synergy.merge(api_version: api_version))
  when 3000
    $client_3000 ||= OneviewSDK::Client.new($config.merge(api_version: api_version))
    $client_3000_synergy ||= OneviewSDK::Client.new($config_synergy.merge(api_version: api_version))
  end
