...
module OneviewSDK
  ...
  module API2400
    ...
    # Get the current API2400 variant
    def self.variant
      @variant
    end

    # Has the API2400 variant been set by the user?
    # @return [TrueClass, FalseClass]
    def self.variant_updated?
      @variant_updated
    end

    # Sets the API2400 variant
    def self.variant=(variant)
      raise "API2400 variant '#{variant}' is not supported! Try one of #{SUPPORTED_VARIANTS}" unless SUPPORTED_VARIANTS.include?(variant)
      @variant_updated = true
      @variant = variant
    end

    # Helps redirect resources to the correct API2400 variant
    def self.const_missing(const)
      api2400_module = OneviewSDK::API2400.const_get(@variant.to_s)
      api2400_module.const_get(const)
    rescue NameError
      raise NameError, "The #{const} method or resource does not exist for OneView API2400 variant #{@variant}."
    end
  end
end

# Load all API2400-specific resources:
