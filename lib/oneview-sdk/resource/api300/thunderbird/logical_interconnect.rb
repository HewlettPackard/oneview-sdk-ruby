...
module OneviewSDK
  module API300
    module Thunderbird
      # Logical interconnect resource implementation for API300 Thunderbird
        ...
        def list_vlan_networks
          ...
          results.each do |vlan|
            net = if vlan['generalNetworkUri'].include? 'ethernet-network'
                    OneviewSDK::API300::Thunderbird::EthernetNetwork.new(@client, uri: vlan['generalNetworkUri'])
                  elsif vlan['generalNetworkUri'].include? 'fc-network'
                    OneviewSDK::API300::Thunderbird::FCNetwork.new(@client, uri: vlan['generalNetworkUri'])
                  else
                    OneviewSDK::API300::Thunderbird::FCoENetwork.new(@client, uri: vlan['generalNetworkUri'])
                  end
            net.retrieve!
